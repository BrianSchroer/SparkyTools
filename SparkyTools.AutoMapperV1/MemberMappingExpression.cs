    ...
    public class MemberMappingExpression<TSource, TDestination>
    {
        ...
        public IMappingExpression<TSource, TDestination> UseValue<TResult>(TResult value)
        {
            if (typeof(TResult) == _dateTimeType)
            {
                ...
                {
                    return UseValue(() => DateTime.Today);
                }

                if ((dt - DateTime.Now).TotalSeconds < 1)
                {
                    return UseValue(() => DateTime.Now);
                }
            }

            return _mappingExpression.ForMember(_member, opt => opt.MapFrom(_ => value));
        }
    }
}
